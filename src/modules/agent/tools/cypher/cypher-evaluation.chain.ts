...
// NOTE: Use double braces ({{ and }}) to escape braces that are not text placeholders.
// NOTE: "No preamble" could maybe be used to just get the Cypher back in other cases as well.
const cypherEvalPromptText = `
...
export default async function initCypherEvaluationChain(
  ...
  const cypherEvalPrompt = PromptTemplate.fromTemplate( cypherEvalPromptText )

  // The chain will recursively verify using the output described in the prompt,
  // which includes an array of errors.
  return RunnableSequence.from<CypherEvaluationChainInput,CypherEvaluationChainOutput>([
    // The prompt will need these in string format, so as the first step, use the
    // RunnablePassthrough.assign() method to convert the array of errors into a
    // single string.
    ...
    //Now that you have the inputs that the prompt expects, update the chain
    // to format the prompt, pass it to the LLM to process and parse the output.
    cypherEvalPrompt,
    llm,
